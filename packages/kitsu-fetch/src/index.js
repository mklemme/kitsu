...
export default class Kitsu {
  ...
  async get (model, params = {}, headers = {}) {
    try {
      ...
      const response = await this.request(`${this.baseURL}/${url}${query(params)}`, {
        method: 'GET',
  ...
  async patch (model, body, headers = {}) {
    try {
      ...
      const response = await this.request(`${this.baseURL}/${url}/${body.id}`, {
        method: 'PATCH',
        headers: Object.assign(this.headers, headers),
  ...
  async post (model, body, headers = {}) {
    try {
      ...
      const response = await this.request(`${this.baseURL}/${url}`, {
        method: 'POST',
        headers: Object.assign(this.headers, headers),
        body: JSON.stringify(serialise(resourceModel, body, 'POST', {
          camelCaseTypes: this.camel,
          pluralTypes: this.plural
        }))
      })
  ...
  async delete (model, id, headers = {}) {
    try {
      ...
      const response = await this.request(`${this.baseURL}/${path}`, {
        method: 'DELETE',
        headers: Object.assign(this.headers, headers),
        body: JSON.stringify(serialise(resourceModel, payload, 'DELETE', {
          camelCaseTypes: this.camel,
          pluralTypes: this.plural
        }))
      })
  ...
  async request ({ body, method, params, type, url }, headers = {}) {
    try {
      ...
      const response = await this.request(`${this.baseURL}/${url}${query(params)}`, {
        method,
        headers: Object.assign(this.headers, headers),
        body: JSON.stringify([ 'GET', 'DELETE' ].includes(method) ? undefined : serialise(type, body, method, {
          camelCaseTypes: this.camel,
          pluralTypes: this.plural
        }))
      })
